...
用于管理 Linux BBR、Snell Server、shadowsocks-rust（SS-2022）和 ShadowTLS v3 的独立 Bash 脚本。组件只从各自官方发布源下载，不执行第三方安装脚本。

## 主要功能

- Snell、SS-2022、ShadowTLS 均支持 `1–99` 多实例。
- 首次安装直接回车使用实例 `1`。
...
SS-2022 与 ShadowTLS 使用官方 musl 静态构建，适合 Alpine。Snell 官方 Linux 二进制通常要求 glibc；在 Alpine/musl 上可能需要兼容层，脚本会在二进制无法执行时给出明确错误。

建议至少 256 MiB 内存。依赖按组件延迟安装：Snell 不需要 `jq`/`xz`；SS-2022 与 ShadowTLS 使用 `jq` 解析 GitHub 官方发布信息，SS-2022 还使用 `jq` 管理 JSON 配置。
...
- systemd：`newworld-snell-<实例>.service`、`newworld-ss2022-<实例>.service`、`newworld-shadowtls-<实例>.service`
