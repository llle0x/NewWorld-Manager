...
安装完成后及菜单“查看配置”都会输出完整配置块：Snell 包含可放进 Surge `[Proxy]` 的配置行和服务端配置；SS-2022 包含可导入的 `ss://` 链接和服务端 JSON；ShadowTLS 包含完整端点、密码、SNI 和后端信息。若 Snell 或 SS-2022 已套用 ShadowTLS，客户端配置会自动改用 ShadowTLS 外部端口并加入 ShadowTLS v3 参数。请妥善保存，不要公开分享密钥。

Snell、SS-2022 与 ShadowTLS 都支持多实例：每次安装可指定 `1-99` 的实例编号，每个实例拥有独立端口、配置文件及 systemd 服务。ShadowTLS 会明确绑定到某个 Snell 或 SS-2022 实例；同一后端实例也可套多个 ShadowTLS。原有单实例会在首次相关操作时自动迁移为实例 `1`。

更新 Snell 或 SS-2022 时，输入新编号会安装新实例，输入已有编号会更新该实例；直接回车会更新全部已安装实例。ShadowTLS 输入新编号会安装新实例，直接回车会更新全部实例。
